...
def join_room(room_id,uid,password,name):
    if room_id not in rooms: return False,"No room"
    if rooms[room_id]["password"]!=password: return False,"Wrong password"
...
def remove_player(room_id,uid):
    ...
    if uid==r["host"]:
        if r["players"]: r["host"]=next(iter(r["players"]))
        else: rooms.pop(room_id)
